...
const formatMap = new Map<string, FormatInstruction>([
    ...
    [
        "gpsLongitude",
        { category: "exif", displayName: "GPS Longitude", formatFunc: formatLongitude }
    ],
    ...
    [
        ...
        { category: "maker", displayName: "Flash Exposure Compensation" }
    ],
    ...
    [
        "hyperfocalDistance",
        { category: "composite", displayName: "Hyperfocal Distance", formatFunc: distance }
    ],
    ...
    Object.keys(exif).forEach(key => {
        ...
        if (val) {
            ...
            if (cfg && cfg.category === category) {
                const formattedValue = cfg.formatFunc
                    ? cfg.formatFunc(val as number)
                    : val.toString();
