...
const BulkEditGpsCard: Component<Props> = (props) => {
    ...
    return (
        <div class="mx-4">
            <div>
                <input type="text" class="input input-sm w-300px" placeholder="Latitude" classList={getValidationClass(override().lat)} onPaste={onPaste} value={override().lat ?? ""} onInput={evt => setOverride(prev => ({ lat: evt.currentTarget.value, lng: prev.lng}))}  />
            </div>

            <div class="mt-2">
                <input type="text" class="input input-sm w-300px" placeholder="Longitude" classList={getValidationClass(override().lng)} onPaste={onPaste} value={override().lng ?? ""} onInput={evt => setOverride(prev => ({ lat: prev.lat, lng: evt.currentTarget.value }))} />
            </div>

            <div class="mt-4">
                <button class="btn btn-sm mr-2" classList={getButtonClass()} disabled={!isOverrideValid()} onClick={save}>Save</button>
