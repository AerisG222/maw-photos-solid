...
export const useCategoriesByYear = () => {
    ...
    const { categoriesForAllYearsQuery, categoriesWithoutGpsForAllYearsQuery, setIsFavoriteMutation } = useCategoriesContext();
    ...
    const allCategoriesReady = () => allCategories() && !allCategories()!.some(result => result.isPending);

    const [categoryIdsWithoutGps] = createResource(
        () => ({ yearFilter: filter.yearFilter, gpsFilter: filter.missingGpsFilter, yearsReady: years.isSuccess }),
        () => {
            if (filter.yearFilter === "all") {
                if (years.isSuccess) {
    ...
    const categoryIdsWithoutGpsReady = () => categoryIdsWithoutGps() && !categoryIdsWithoutGps()!.some(result => result.isPending);

    const filterCategoriesWithoutGps = (categoriesForYear: Category[], categoryIdsWithoutGps: Uuid[] | undefined) =>
        categoriesForYear.filter(cat => !!categoryIdsWithoutGps?.find(id => cat.id === id));

    const getCategoryIdsWithoutGpsForYear = (year: number, categoryIdsWithoutGpsResult: UseQueryResult<CategoryIdsForYearResult, Error>[]) =>
        categoryIdsWithoutGpsResult
            .find(x => x.data?.year === year)
            ?.data?.categoryIds;
                ...
                return allCategories()!.reduce<Record<number, Category[]>>((acc, result) => {
                    if (result.data) {
                        acc[result.data.year] = filterCategoriesWithoutGps(
                            ...
                            getCategoryIdsWithoutGpsForYear(result.data.year, categoryIdsWithoutGps.latest!)
                        );
                    }
                    return acc;
                }, {});
